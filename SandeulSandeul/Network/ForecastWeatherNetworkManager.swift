...
final class ForecastWeatherNetworkManager {
    
    static let shared = ForecastWeatherNetworkManager()
    ...
    func fetchNetwork(x: Int, y: Int, baseTime: String) -> AnyPublisher<ForecastWeather, Never> {
        ...
        return Future<ForecastWeather, Never> { promise in
            AF.request(url).validate().responseDecodable(of: ForecastWeather.self) { response in
