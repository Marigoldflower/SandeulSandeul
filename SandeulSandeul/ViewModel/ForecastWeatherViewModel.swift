...
final class ForecastWeatherViewModel {
    ...
    @Published var forecastWeatherPublisher: ForecastWeather?
    ...
    func fetchWeatherNetwork(x: Int, y: Int, baseTime: String) {
        ...
        ForecastWeatherNetworkManager.shared.fetchNetwork(x: x, y: y, baseTime: baseTime).sink { [weak self] forecastWeather in
            self?.forecastWeatherPublisher = forecastWeather
