//
//  ThreeDaysForecastViewModel.swift
//  SandeulSandeul
//
...
final class ThreeDaysForecastViewModel {
    
    var cancellables: Set<AnyCancellable> = []
    
    // ForecastWeather 타입에 변화가 생기면 View에도 영향이 갈 수 있게끔 값을 바인딩한다.
    @Published var threeDaysPublisher: ThreeDaysForecast?
    ...
    func fetchWeatherNetwork(latitude: String, longtitude: String) {
        ...
        ThreeDaysForecastNetworkManager.shared.fetchNetwork(latitude: latitude, longtitude: longtitude).sink { [weak self] threeDays in
            self?.threeDaysPublisher = threeDays
